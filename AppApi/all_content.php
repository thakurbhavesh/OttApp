...
$query = "SELECT 
    c.content_id,
    c.title, c.thumbnail_url, c.trailer_url, c.plan AS plan_type, c.industry,
...
if ($language_id) { $query .= " AND c.language_id = ?"; $params[] = $language_id; $types .= "i"; }
if ($preference_id) { $query .= " AND c.preference_id = ?"; $params[] = $preference_id; $types .= "i"; }
if ($category_id) { $query .= " AND c.category_id = ?"; $params[] = $category_id; $types .= "i"; }
if ($main_category_id) { $query .= " AND cat.main_category_id = ?"; $params[] = $main_category_id; $types .= "i"; }
if ($plan) { $query .= " AND c.plan = ?"; $params[] = $plan; $types .= "s"; }
if ($industry) { $query .= " AND c.industry = ?"; $params[] = $industry; $types .= "s"; }
...
try {
    ...
    if (empty($contents)) {
        echo json_encode(['status' => 'success', 'message' => 'No content found', 'data' => []], JSON_PRETTY_PRINT);
    } else {
